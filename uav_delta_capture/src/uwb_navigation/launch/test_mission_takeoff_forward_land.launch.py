#!/usr/bin/env python3
"""Prop-on GUIDED takeoff, local-position forward move, hover, and LAND test."""
import os
...
def generate_launch_description():
    ...
    test_mission_node = Node(
        ...
        parameters=[os.path.join(share_dir, 'test_mission_takeoff_forward_land.yaml')],
        output='screen',
    )
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[
                    EmitEvent(
                        event=Shutdown(reason='takeoff_forward_land test_mission_node completed')
                    )
                ],
            )
        ),
