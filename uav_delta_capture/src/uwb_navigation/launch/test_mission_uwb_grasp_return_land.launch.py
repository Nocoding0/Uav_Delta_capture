#!/usr/bin/env python3
"""Real prop-on UWB approach, grasp wait, return home, drop wait, and LAND test."""
import os
...
def generate_launch_description():
    ...
    test_mission_node = Node(
        ...
        parameters=[os.path.join(share_dir, 'test_mission_uwb_grasp_return_land.yaml')],
        output='screen',
    )
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[
                    EmitEvent(
                        event=Shutdown(reason='uwb_grasp_return_land test_mission_node completed')
                    )
                ],
            )
        ),
