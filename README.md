...
```text
Uav_Delta_capture/
├── models/                  # 预置 ONNX 模型（YOLOv8n）
├── bench_npu.py             # NPU 独立测试脚本
├── bench_standalone.py      # 独立性能测试脚本
└── uav_delta_capture/       # ROS 2 工作空间
    └── src/
...
| 文件 | 节点 | 主要参数 |
|------|------|----------|
| [delta_kinematics.yaml](uav_delta_capture/src/delta_kinematics/config/delta_kinematics.yaml) | delta_kinematics_node | `L1=100`, `L2=150`, `R=55`, `r=20`（mm） |
| [perception_logic.yaml](uav_delta_capture/src/perception_logic/config/perception_logic.yaml) | perception_node | `image_topic`, `publish_topic`, `camera_frame` |
| [uav_bridge.yaml](uav_delta_capture/src/uav_bridge/config/uav_bridge.yaml) | uav_bridge_node | `serial_port`, `baudrate`, `camera_frame`, `target_frame` |
| [mock_mavros_pose.yaml](uav_delta_capture/src/uav_bridge/config/mock_mavros_pose.yaml) | mock_mavros_pose_node | `trajectory_mode`, `rate`, `origin_*`, `amplitude_*` |
| [fcu_health.yaml](uav_delta_capture/src/uav_bridge/config/fcu_health.yaml) | fcu_link_monitor / flight_state_machine | `timeout_sec`, `threshold_count`, `publish_rate_hz` |
| [failsafe.yaml](uav_delta_capture/src/uav_bridge/config/failsafe.yaml) | failsafe_manager_node | `input_timeout_sec=0.4`, `safe_x/y/z` |
| [mavros_bridge.yaml](uav_delta_capture/src/uav_bridge/config/mavros_bridge.yaml) | mavros_node | `fcu_url`, `system_id`, `component_id` |
| [vision_bench.yaml](uav_delta_capture/src/vision_bench/config/vision_bench.yaml) | vision_bench_node | `input_size`, `use_npu`, `num_iterations` |
